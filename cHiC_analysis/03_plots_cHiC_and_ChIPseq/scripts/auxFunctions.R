...
##########################
intervals1DInclusion <- function(interval1DA,interval1DB){
    # Returns 0 if interval1DA includes completely interval1DB
    # Returns 1 otherwise


    if(interval1DA$chrom != interval1DB$chrom){return(1)}
    if(interval1DA$start >  interval1DB$start){return(1)}
    if(interval1DA$end   <  interval1DB$end  ){return(1)}    
...
extractChIPData <- function(chipTarget,coords,extend=0,vType='avg'){
	...
	chipTrack <- gtrack.ls("chipseq",chipTarget,"IJ")
...
plotChIP <- function(chipData, chipStart=NA, plotOrder=NA, plotCols=NA, plotTitles=NA, vertical=FALSE, chipRes=10, plotAnn=F, plotStart=F, plotEnd=F, inputType='avg', cex=1, plotMax=NA, plotMin=NA, main=TRUE, adj = 0.5, plotAxis=TRUE, CTCFDir=NA, peaksDir=NA, chrom=NA, condition=NA, peakSet=NA){

	print("Ordering ChIPseqs: Start")
        chipOrder <- plotOrder
	print(colnames(chipData))
        cNames  <- colnames(chipData)[-grep('chrom|start|end|intervalID', colnames(chipData))]	
	#cNames  <- gsub('chipseq.|vChip|cnr.|GSproject.|insulation.','',cNames)
	#cNames  <- gsub('_merge_mm10|_dm6|_GS','',cNames)
	print(paste0("chip Names ",cNames))
	...
	{
	    print(chip)
	    ind <- print(grep(chip,cNames))
	    #print(gsub('insulation.|_merge_mm10_IJ|_INS','',cNames))
	    print(paste0(chip," ",ind))
	    #quit()
	    if(length(ind) > 0)
	    {
 	        cName <- cNames[grep(chip,cNames)] 
		max   <- plotMax[grep(chip,chipOrder)]
		min   <- plotMin[grep(chip,chipOrder)]	    		
		title <- plotTitles[grep(chip,chipOrder)]	    

		cNamesOrdered     <- append(cNamesOrdered,cName)
	    	plotMaxOrdered    <- append(plotMaxOrdered,max)
	    	plotMinOrdered    <- append(plotMinOrdered,min)		
		plotTitlesOrdered <- append(plotTitlesOrdered,title)
		print(paste(cName,max,title))
	    }
	    else
	    {
	        cName <- "NotAvailable"
        ...
        print("Ordering ChIPseqs: End")
	cNames     <- cNamesOrdered
	plotMax    <- unlist(plotMaxOrdered)
	plotMin    <- unlist(plotMinOrdered)	
	plotTitles <- plotTitlesOrdered	
        ...
        for (chip in cNames)
	{
		...
		if (length(grep(chip,colnames(chipData))) == 0)
		{
		    #plot.new()
		    next
		}
		
		#if (length(grep(paste0(chip,"_"),colnames(chipData))) > 1){
		if (length(grep(chip,colnames(chipData))) > 1){		
			...
			chip <- gsub('chipseq.|v_chipseq.|insulation.','',chip)
			#print(chip)
		}else{
		...
		currentData[is.na(currentData)] <- 0
                print(max(currentData))
                if(inputType == 'log'){currentData <- log2(currentData+1)}
		if(inputType == 'gpm'){currentData <- -log2(1-currentData)}
		
		if(length(grep('INS',chip)) > 0)
		{
		    print(currentData)
		    insMax <-  max(-currentData)
                    insMin <-  min(-currentData)
                    print(paste0(insMin,' ',insMax))
		    currentData <- (-currentData - insMin) / (insMax - insMin)
		    print(head(currentData))
		    print(paste0(chipStart," ",plotStart," ",as.integer((chipStart-plotStart)/chipRes)))
		    currentData <- currentData[as.integer((plotStart-chipStart)/chipRes):as.integer((plotEnd-chipStart)/chipRes)]
		    #currentData <- currentData[plotStart <= currentData$start & currentData$start <= plotEnd,]
		    print(head(currentData))
		    #quit()
		}
		...
		if(main==TRUE){
		    title <- gsub('_',' ',chip)
		    print(paste0("Title from chip-name: ",title))
		}else{
		    title <- ''
		}
		if(!is.na(plotTitles[1])){
		    title <- plotTitles[which(cNames == chip)]
		    print(paste0("Title from user: ",title))
		}
		#print(paste0("Title ",title))


		if(vertical == TRUE){
			print(colnames(currentData))
			barplot(currentData, cex.main=1.6, xlab='', ylab='', yaxt='n', xaxt='n', lwd=1.5, col=col, border=col, main=gsub('v_chipseq.|v_bb_chip|v_hic.','',chip), adj = adj, horiz=T, space=0)
			...
			if(is.data.frame(plotAnn))
			{
				...
				{
					row <- plotAnn[g,]
					rect(0,(row$start-plotStart)/chipRes, 1000, (row$end-plotStart)/chipRes, col=rgb(0.66,0.66,0.66, alpha=0.2), border = 0)
				}
			}
		}else{
			if(!is.na(plotMax[1])){
				#print(plotMax[which(cNames == chip)])
				yLim <- c(as.numeric(plotMin[which(cNames == chip)]),as.numeric(plotMax[which(cNames == chip)]))
				#yLim <- c(0,max(currentData))				
				#     				yLim <- c(0,25)
				print(yLim)
				barplot(currentData, cex.main=cex*1.2, xlab='', ylab='', yaxt='n', xaxt='n', lwd=1.5, col=col, border=col, main=title, xpd = F, space=0, ylim=yLim, adj = adj)

				# Plot identified peaks as points                                
				print(chip)
				
				#peakFile <- paste0(peaksDir,gsub('chipseq.|_merge_mm10_IJ','',chip),'_merge_mm10_IJ_peak_summits.bed')
				#chic_NPC_WTmm10_IJ_INS_Scale_300kb_Res_500bp
				print(unlist(strsplit(chip,'_'))[7])
				print(unlist(strsplit(chip,'_'))[9])				
				#peakFile <- paste0(peaksDir,'Insulation_maxima_chic_',condition,'_merge_mm10_IJ_INS_Scale_',unlist(strsplit(chip,'_'))[7],'_Res_',unlist(strsplit(chip,'_'))[9],'_',peakSet,'.bedGraph')
				peakFile <- paste0(peaksDir,'Insulation_maxima_chic_',condition,'_merge_mm10_IJ_INS_Scale_200bk_Res_500bp_aboveMean.bedGraph')				
				print(peakFile)
				...
				{
				    print(peakFile)
                                    plotPeaks <-read.table(peakFile, header=T)
				    colnames(plotPeaks) <- c("chrom","start","end")
				    print(head(plotPeaks))
				    tolerance <- 10
				    for(a in (as.integer(plotPeaks$start)-plotStart)/500)
				    {
				        print(paste0("Maxima at ",a))
					rect(xleft=a-tolerance, xright=a+tolerance, ybottom=0, ytop=max(currentData), density=20, col="yellow")					
				    }
				    next

				    #plotPeaks <- plotPeaks[plotPeaks$chrom == chrom & plotPeaks$start >= plotStart & plotPeaks$end <= plotEnd,]
				    #print(head(plotPeaks))
				    
				    for(g in 1:nrow(plotPeaks))
				    {					
				        row <- plotPeaks[g,]
					print(row)
					bin <- round_any((((row$start+row$end)/2)-plotStart)/chipRes,1)
					occupation <- append(occupation,bin)	
					points(x=bin, y=yLim[[2]], pch=19, col=2)	     
				    }

				# Plot motif directionality for CTCF
				CTCFDir <- peaksDir
				ind <- print(grep("CTCF",chip))  
				if(!is.na(CTCFDir))
				{
				    if(length(ind) > 0)
				    {
				    occupationF <- c()
				    occupationR <- c()				    
				    print("Adding CTCF directionality information")

				    arrowFile <- peakFile
				    plotArrows <- read.table(arrowFile, header=F)
				    colnames(plotArrows) <- c("chrom","start","end","score","direction")					    
				    plotArrows <- plotArrows[c('chrom','start','end','score','direction')]
				    plotArrows <- plotArrows[plotArrows$chrom == chrom & plotArrows$start >= plotStart & plotArrows$end <= plotEnd,]
				    print(head(plotArrows))

				    for(g in 1:nrow(plotArrows))
                                    {
					row <- plotArrows[g,]
					yLevel <- yLim[[2]]/2
					...
					print(bin)
					print(occupation)
					if(is.element(bin,occupation))
					{
					    if (row$direction == "+" & !is.element(bin,occupationF))
					    {					        
					        # Plot forward motifs
						occupationF <- append(occupationF,bin)								
					        print(row)
						arrows(x0=bin, y0=yLevel, x1=bin+1000,y1=yLevel, angle=30, col=2, length=0.3*10*(1/96), lwd = 2, code=2)
					    }
					    if (row$direction == "-" & !is.element(bin,occupationR))
					    {
	 				        # Plot reverse motifs
						occupationR <- append(occupationR,bin)						
					        print(row)
					        arrows(x0=bin-1000, y0=yLevel, x1=bin,y1=yLevel, angle=30, col=2, length=0.3*10*(1/96), lwd = 2, code=1)
					    }
					}
                                    }
				}
				...
				#else {
				#    print(paste0("Can't plot peaks. ",peakFile," doesn't exist!"))
				#}


				if(plotAxis == TRUE){
					axis(2, at=yLim, labels=signif(yLim,2), cex.axis=cex, las=1)
					#axis(1, at=c(0,length(currentData)), labels=signif(c(chipStart/1000000,(chipStart+length(currentData)*chipRes)/1000000),2), cex.axis=cex, las=1)
					axis(1, at=c(0,length(currentData)), labels=c("",""), cex.axis=cex, las=1)
					}
			}else{
			        barplot(currentData, cex.main=cex*1.2, xlab='', ylab='', yaxt='n', xaxt='n', lwd=1.5, col=col, border=col, main=title, space=0, adj = adj)
				if(plotAxis == TRUE){
...
plotIns <- function(chipData, chipStart=NA, plotOrder=NA, plotCols=NA, plotTitles=NA, vertical=FALSE, chipRes=10, plotAnn=F, plotStart=F, plotEnd=F, inputType='avg', cex=1, plotMax=NA, main=TRUE, adj = 0.5, plotAxis=TRUE, CTCFDir=NA, peaksDir=NA, chrom=NA, condition=NA, peakSet=NA){
        ...
        for (chip in cNames)
	{
		...
		if(vertical == TRUE){
			...
			if(!is.na(plotMax[1])){
				#print(plotMax[which(cNames == chip)])
				#yLim <- c(0,as.numeric(plotMax[which(cNames == chip)]))
				#				yLim <- c(0,max(currentData))
				#yLim <- c(min(currentData),max(currentData))
				yLim <- c(1.8,3.0)												
				#     				yLim <- c(0,25)
				print(yLim)
				plot(currentData, cex.main=cex*1.2, type='l', xlab='', ylab='', yaxt='n', xaxt='n', lwd=1.5, col=col, border=col, axes=F, main=title, xpd = F, space=0, ylim=yLim, adj = adj)
...
plotVirt4C <- function(chipData, plotOrder=NA, plotCols=NA, plotTitles=NA, vertical=FALSE, chipRes=10, plotAnn=F, plotStart=F, plotEnd=F, inputType='avg', cex=1, plotMax=NA, main=TRUE, adj = 0.5, plotAxis=TRUE, CTCFDir=NA, peaksDir=NA, chrom=NA){

	#cNames <- colnames(chipData)[-grep('chrom|start|end|intervalID', colnames(chipData))]

	print("Ordering Virtual 4Cs: Start")
        chipOrder <- plotOrder
        cNames <- colnames(chipData)[-grep('chrom|start|end|intervalID', colnames(chipData))]
	print(cNames)
	cNamesOrdered  <- c()
	...
	for(chip in chipOrder)
	{
		...
		max   <- plotMax[grep(chip,chipOrder)]	    
	    	cNamesOrdered  <- append(cNamesOrdered,cName)
	    	plotMaxOrdered <- append(plotMaxOrdered,max)
		print(paste(cName,max))
	    }
	}
        print("Ordering Virtual 4Cs: End")
	cNames  <- cNamesOrdered
	plotMax <- plotMaxOrdered
	print(cNames)
	print(plotMax)
	cNames  <- gsub('chipseq.|vChip|cnr.','',cNames)
	cNames  <- gsub('_merge_mm10_IJ|_merge_dm6_GS','',cNames)
	print(cNames)
	#quit()
        ...
        for (chip in cNames)
	{
		...
		#		if (length(grep(paste0(chip,),colnames(chipData))) > 1){
		if (length(grep(paste0(chip),colnames(chipData))) > 1){		
			allChIP <- colnames(chipData)[grep(chip,colnames(chipData))]
			chip <- allChIP[which(nchar(allChIP) == min(nchar(allChIP)))]
			currentData <- chipData[,which(colnames(chipData) == chip)]
			...
			currentData <- chipData[,grep(paste0(chip), colnames(chipData))]
		}
		print(head(currentData))
		#		quit()
		...
		if(vertical == TRUE){
			...
			if(!is.na(plotMax[1])){
				#print(plotMax[which(cNames == chip)])
				yLim <- c(-100,as.numeric(plotMax[which(cNames == chip)]))
				#     				yLim <- c(0,25)
				print(yLim)
				print(currentData)
				barplot(currentData, cex.main=cex*1.2, xlab='', ylab='', yaxt='n', xaxt='n', lwd=1.5, col=col, border=col, main=title, xpd = F, space=0, ylim=yLim, adj = adj)
				# Plot identified peaks as points                                
				print(chip)
				#peakFile <- paste0(peaksDir,chip,'_merge_mm10_IJ_peak_summits.bed')
				peakFile <-  paste0(peaksDir,chip,'_merge_mm10_IJ_top_50perc_peak_summits.bed')
				#print(peakFile)
				...
				{
				    print(peakFile)
                                    plotPeaks <-read.table(peakFile, header=F)
				    colnames(plotPeaks) <- c("chrom","start","end")
				    plotPeaks <- plotPeaks[plotPeaks$chrom == chrom & plotPeaks$start >= plotStart & plotPeaks$end <= plotEnd,]
				    print(head(plotPeaks))
				...
				if(!is.na(CTCFDir))
				{
				    ...
				    plotArrows <- read.table(arrowFile, header=T)
				    plotArrows <- plotArrows[c('chrom','start','end','direction')]
				    plotArrows <- plotArrows[plotArrows$chrom == chrom & plotArrows$start >= plotStart & plotArrows$end <= plotEnd,]
				    print(head(plotArrows))
					...
					#bin    <- round_any((((row$start+row$end)/2)-plotStart)/chipRes,1)
					bin    <- round_any((row$start-plotStart)/chipRes,1)
					print(bin)
					...
					if(is.element(bin,occupation))
					{
						...
						occupationF <- append(occupationF,bin)								
					        print(row)
						arrows(x0=bin, y0=yLevel, x1=bin+10,y1=yLevel, angle=30, col=2, length=0.3*10*(1/96), lwd = 2, code=2)
					    }
					    if (row$direction == "-" & !is.element(bin,occupationR))
					    {
	 				        # Plot reverse motifs
						occupationR <- append(occupationR,bin)						
					        print(row)
					        arrows(x0=bin-10, y0=yLevel, x1=bin,y1=yLevel, angle=30, col=2, length=0.3*10*(1/96), lwd = 2, code=1)
					    }
					}

					bin    <- round_any((row$end-plotStart)/chipRes,1)					
				...
				if(plotAxis == TRUE){
			        ...
			        barplot(currentData, cex.main=cex*1.2, xlab='', ylab='', yaxt='n', xaxt='n', lwd=1.5, col=col, border=col, main=title, adj = adj, space=0)
				if(plotAxis == TRUE){
					axis(2, at=c(0,max(currentData,na.rm=TRUE)), labels=c(0,signif(max(currentData,na.rm=TRUE),2)), cex.axis=cex, las=3)
				}
			}
# 		axis(2, cex.axis=cex, las=2)
		...
		if(!is.null(plotAnn)){		
 			print('plotting annotation')
			row <- as.data.frame(unlist(plotAnn[chip]))
			print(row)
			rownames(row) <- c("chrom","start","end")
			print(row)
			s1 = as.integer(row["start",])
			s2 = as.integer(row["end",])			
			rect(xleft=(s1-plotStart)/chipRes, ybottom=-100, xright=(s2-plotStart)/chipRes, ytop=100, col="yellow", density=20)
		}
		}
	}
}
...
#genesPlot <- function(interval, xend, vertical=FALSE, header='test', cex=1.4, rHeight=40){
#    mart = useMart('UCSF', dataset='mouse_gene_ucsf')
#    genesplus = makeGeneRegion(start = interval$start, end = interval$end, strand = '+', chromosome = interval$chrom, biomart = mart)
#    genesmin  = makeGeneRegion(start = interval$start, end = interval$end, strand = '-', chromosome = interval$chrom, biomart = mart)
...
# 		par(mar = c(0, plotMar[1], 0, plotMar[2]))
		...
		for(g in 1:nrow(set1))
		{
			...
			if(row$strand == 1){
				...
				rect(row$start, minH, row$end, rHeight, col=rgb(158/255,158/255,158/255, alpha=1.), border = NA); currentX=row$start; textY <- arH; textPos <- 3;
				...
				rect(row$start, -minH, row$end, -rHeight, col=rgb(69/255,69/255,69/255, alpha=1.), border = NA); currentX=row$start; textY <- arH; textPos <- 3;
